...
SALMO - que sea señor mi oración.m4a	https://drive.google.com/file/d/1k9I1M9IJCjU-lnbM7Nf0DIhzZV1JgIca/view?usp=drivesdk	1k9I1M9IJCjU-lnbM7Ff0DIhzZV1JgIca	audio/x-m4a
...
def clean_title(title):
    # Remover extensiones
    title = re.sub(r'\.(pdf|m4a|mp4|mp3|mpeg|x-m4a)$', '', title, flags=re.IGNORECASE)
    # Remover "Copy of "
    title = title.replace('Copy of ', '')
    # Remover prefijos de momentos
    for prefix in mapa_tipos.keys():
...
for line in lines:
    ...
    full_name, url, file_id, mime = parts
    
    # Determinar tipo_canto_id
    tipo_id = 12 # Default Meditacion
    for prefix, tid in mapa_tipos.items():
    ...
    if 'pdf' in mime.lower():
        cantos[title]['url_pdf'] = f"'{url}'"
    else:
        cantos[title]['url_audio'] = f"'{url}'"

# Generar SQL
sql_output = "INSERT INTO public.cantos (titulo, tipo_canto_id, url_pdf, url_audio) VALUES\n"
values = []
for c in cantos.values():
    t = c['titulo'].replace("'", "''")
    values.append(f"  ('{t}', {c['tipo_id']}, {c['url_pdf']}, {c['url_audio']})")

sql_output += ",\n".join(values) + ";"

print(sql_output)

with open('c:/ECOSAT/Proyectos AI/Proyectos AI/CantosCuu/supabase/migrations/06_ingesta_masiva.sql', 'w', encoding='utf-8') as f:
    f.write(sql_output)
